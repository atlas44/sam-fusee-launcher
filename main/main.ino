...
void sleep(int errorCode){
    ...
    digitalWrite(13, LOW);
    if(errorCode == 1){
          strip.setPixelColor(0, 0, 0, 0); strip.show(); //led to off
    }else{
          strip.setPixelColor(0, 64, 0, 0); strip.show(); //led to off
    }
...
void setup()
{
    ...
    strip.begin();
    strip.setPixelColor(0, 0, 64, 0); strip.show(); //led to green
    ...
    while (!foundTegra)
    {
        ...
        if (currentTime > lastCheckTime + 100){
              if(blink){
                strip.setPixelColor(0, 64, 32, 0); strip.show(); //led to orange
              }else{
                strip.setPixelColor(0, 0, 0, 0); strip.show(); //led to black
              }
              blink = !blink;
